...
`SymbolTableResolver` and `SeparateDebugResolver` compile the filter before
opening any binary; with `--scope project`, `GoProjectResolver` first reads the
Go build info from the binary to find the module path and only then delegates
to `SymbolTableResolver`. The filter's `shouldInclude` applies, in order:
symbol binding exclude, symbol binding include (library API only), `--exclude`
regex, `--include` regex. A name pattern matches when it matches the raw or
