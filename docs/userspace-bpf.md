...
> [changelog](https://github.com/maxgio92/xcover/releases) for the first
...
- **`--pid` is rejected.** bpftime at the pinned commit stores the PID filter
  ...
  --userspace-bpf`.
- **No map-full warning.** The `seen_funcs` drop counter and the
  `calls not recorded because the seen_funcs map is full` warning only work
  in kernel mode. bpftime's default hash map never reports that it is full,
  so past 40960 functions coverage undercounts without notice.
